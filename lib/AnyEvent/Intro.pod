...
event, which is fast and doesn't block your process. Only when data is
available for reading will the callback be called, which can then proceed
to read the data.

The "interest" is represented by an object returned by C<< AnyEvent->io
>> called a "watcher" object - called this because it "watches" your
