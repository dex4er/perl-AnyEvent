...
If you run this, your program will exit almost instantly. The reason is
that creating the timer does not wait for it, instead the C<timer> method
returns instantly and perl executes the rest of the program.

But there is nothing left to execute, so perl exits.

To wait for an event, use a condvar:
